...
function safe(id: string, name: string, fn: () => NetCheck): NetCheck {
  try { return fn(); } catch (e: any) { return { id, name, status: 'unknown', detail: e.message, risk: 'medium' }; }
}

export function checkWFP(): NetCheck {
  return safe('net-wfp', 'WFP Kernel-Filter', () => {
    const out = ps(`$b=(Get-Service BFE -EA SilentlyContinue).Status;$m=(Get-Service MpsSvc -EA SilentlyContinue).Status;$p=(Get-NetFirewallProfile|?{$_.Enabled}).Count;"BFE:$b|MpsSvc:$m|Profiles:$p/3"`);
...
export function checkGeoIP(): NetCheck {
  return safe('net-geoip', 'Geo-IP Blocking', () => {
    const out = ps(`$r=Get-NetFirewallRule -DisplayName '*GeoBlock*','*Sentinel*Block*' -EA SilentlyContinue;$c=if($r){($r|Measure).Count}else{0};"Rules:$c"`);
...
export function checkTCPHardening(): NetCheck {
  return safe('net-tcphard', 'TCP Stack Hardening', () => {
    const out = ps(`$t=Get-ItemProperty 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters' -EA SilentlyContinue;"SynProtect:$($t.SynAttackProtect)|DeadGW:$($t.EnableDeadGWDetect)|PMTU:$($t.EnablePMTUDiscovery)"`);
...
export function checkOutboundControl(): NetCheck {
  return safe('net-outbound', 'Process Outbound Control', () => {
    const out = ps(`$b=(Get-NetFirewallProfile|?{$_.DefaultOutboundAction-eq'Block'}).Count;"OutboundBlockProfiles:$b/3"`);
...
export function checkTorBlocking(): NetCheck {
  return safe('net-torblock', 'Tor/Proxy Blocking', () => {
    const out = ps(`$r=Get-NetFirewallRule -DisplayName '*Tor*','*Proxy*Block*' -EA SilentlyContinue;$c=if($r){($r|Measure).Count}else{0};"Rules:$c"`);
...
export function checkPortStealth(): NetCheck {
  return safe('net-stealth', 'Port Stealthing', () => {
    const out = ps(`$i=Get-NetFirewallRule -DisplayName '*ICMPv4*' -EA SilentlyContinue|?{$_.Action-eq'Block'-and$_.Enabled-eq'True'};if($i){'STEALTHED'}else{'VISIBLE'}`);
...
export function checkARPProtection(): NetCheck {
  return safe('net-arp', 'ARP Spoofing Protection', () => {
...
export function checkDomainReputation(): NetCheck {
  return safe('net-domrep', 'Domain Reputation Filter', () => {
    const out = ps(`$np=(Get-MpPreference -EA SilentlyContinue).EnableNetworkProtection;$pua=(Get-MpPreference -EA SilentlyContinue).PUAProtection;"NetworkProtection:$np|PUA:$pua"`);
...
export function checkBeaconing(): NetCheck {
  return safe('net-beacon', 'Beaconing Detection', () => {
    const script = `$ErrorActionPreference='SilentlyContinue'
...
export function checkZeroTrust(): NetCheck {
  return safe('net-zerotrust', 'Zero-Trust Isolation', () => {
    const out = ps(`$pub=(Get-NetFirewallProfile -Name Public).DefaultInboundAction;"PublicInbound:$pub"`);
...
export function checkALG(): NetCheck {
  return safe('net-alg', 'App Layer Gateway', () => {
    const out = ps(`$a=(Get-Service ALG -EA SilentlyContinue).Status;"ALG:$a"`);
...
export function checkSMBKill(): NetCheck {
  return safe('net-smbkill', 'SMB Kill-Switch', () => {
    const out = ps(`$s1=(Get-SmbServerConfiguration -EA SilentlyContinue).EnableSMB1Protocol;$s2=(Get-SmbServerConfiguration -EA SilentlyContinue).EnableSMB2Protocol;"SMBv1:$s1|SMBv2:$s2"`);
...
export function checkNetflow(): NetCheck {
  return safe('net-netflow', 'Netflow / Active Connections', () => {
    const out = ps(`$e=(Get-NetTCPConnection -State Established -EA SilentlyContinue).Count;$l=(Get-NetTCPConnection -State Listen -EA SilentlyContinue).Count;"Established:$e|Listening:$l"`);
...
export function runAllNetworkChecks(): NetCheck[] {
  return [
